...
現在の macOS ビルドは Apple Developer ID で署名されていません。アプリを初めて開く際、macOS Gatekeeper が **"opabrow" は壊れているため開けません。ゴミ箱に入れる必要があります。** と表示することがあります。

これはインターネットからダウンロードした未署名アプリに対する Gatekeeper の通常動作であり、ファイルが実際に破損しているわけではありません。opabrow を「アプリケーション」フォルダにドラッグしたあと、隔離属性を削除してください。

```bash
xattr -cr /Applications/opabrow.app
```

その後、opabrow を通常どおり起動できます。今後のリリースでは署名と公証を追加する予定です。
...
常に手前に表示する際は、クリック透過がデフォルトで有効になります。クリックとスクロールは背面のアプリへ渡されます。フローティングページを操作したい場合は、**表示 > 常に手前のときにクリックを透過** から無効にできます。
