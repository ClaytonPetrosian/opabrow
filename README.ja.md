...
次回以降の署名済み macOS リリースは Developer ID で署名され、Apple の公証を受けます。ダウンロードした DMG を開き、**opabrow** を「アプリケーション」フォルダへドラッグして通常どおり起動してください。ターミナルコマンドは不要です。

現在の `v0.3.0` の macOS アセットはこのリリースパイプラインより前のものです。署名済みリリースの公開後に置き換えられます。
